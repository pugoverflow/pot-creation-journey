...
describe("Logo", () => {
  it("renders the Collctiv alt text", () => {
    ...
    expect(screen.getByRole("img", { name: "Collctiv" })).toBeInTheDocument();
  });
  ...
  it("defaults to height 50 and matching width", () => {
    ...
    expect(image).toHaveAttribute("width", String(expectedWidth(50)));
  });
  ...
  it("scales width proportionally for a custom height", () => {
    ...
    expect(image).toHaveAttribute("width", String(expectedWidth(34)));
  });
  ...
  it("marks the image as priority when requested", () => {
    ...
    expect(screen.getByRole("img")).toHaveAttribute("data-priority", "");
  });
});
